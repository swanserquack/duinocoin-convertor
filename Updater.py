...
if not Path(Updater + "/Convertor.py").is_file():
    tempd = requests.get(
        "https://raw.githubusercontent.com/swanserquack/duinocoin-convertor/main/Convertor.py"
    )
...
if not Path(Updater + "/requirements.txt").is_file():
    tempd = requests.get(
        "https://raw.githubusercontent.com/swanserquack/duinocoin-convertor/main/requirements.txt"
    )
...
if jsonresult == False:
    ...
    tf.close()
    print(Complete)
    jsonresult = filecmp.cmp(
        json1, json2, shallow=False
    )  # Recheck to make sure the update completed and jsonresult is set to true
...
if (
    pyresult == True and jsonresult == True and reqresult == True
):  # After the recheck they all are True and the update is complete
